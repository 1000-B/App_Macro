...
# Step 1: Let the user type food name
typed_food = st.text_input("Enter or Select Food")

# Step 2: Filter matching food names as user types
matching_foods = [f for f in food_data.keys() if typed_food.lower() in f.lower()]

# Step 3: Show a selectbox only if there are matches
if matching_foods:
    food = st.selectbox("Choose from existing foods", options=matching_foods)
else:
    food = typed_food  # If no match, take the typed input
